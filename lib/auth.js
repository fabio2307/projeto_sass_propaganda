...
async function requestPasswordReset(supabase, email) {
    ...
    if (!user || !user.verified) {
        return { success: true }; // evitar vazamento de existência de conta
    }
...
async function resetPassword(supabase, resetToken, newPassword) {
...
async function getUserData(supabase, userId) {
    ...
    const raw = userData || { balance: 0, plan: "free" };
    return {
